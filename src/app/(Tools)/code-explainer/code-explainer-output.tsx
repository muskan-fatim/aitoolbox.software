...
const renderFormattedContent = (content: string): JSX.Element => {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Handle code blocks (```language or ```)
      ...
      elements.push(
        <pre key={currentIndex++} className="bg-gray-100 p-3 rounded border overflow-x-auto mb-3">
    ...
    // Handle numbered lists (1. or 2. etc.)
      ...
      elements.push(
        <ol key={currentIndex++} className="list-decimal list-inside mb-3 space-y-1">
          {listItems.map((item, index) => (
            <li key={index} className="ml-2">{item}</li>
          ))}
        </ol>
                  ...
                  <>
                    ...
                    {isEditing ? (
                      <Textarea
                        ...
                        className="w-full min-h-[300px] resize-y rounded-none"
                        placeholder="Code explanation"
                      />
                    ) : (
                      <div className="text-base p-4 border rounded-md bg-white">
